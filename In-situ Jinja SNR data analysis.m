...
%% =======================
%  HISTOGRAM OF SNR (no Feb) + Normal Distribution Fit
% =======================
figure('Position', [200 200 900 600]);

% Plot histogram (normalized)
histogram(all_snr_nofeb, 50, ...
    'Normalization', 'pdf', ...
...
pd = fitdist(all_snr_nofeb, 'Normal');
x_values = linspace(min(all_snr_nofeb), max(all_snr_nofeb), 300);
y = pdf(pd, x_values);
plot(x_values, y, 'r-', 'LineWidth', 2, 'DisplayName', 'Normal Fit');

% Plot mean and median lines
xline(mean_snr, 'r--', 'LineWidth', 2, 'DisplayName', sprintf('Mean = %.2f', mean_snr));
xline(median_snr, 'k--', 'LineWidth', 2, 'DisplayName', sprintf('Median = %.2f', median_snr));
...
statsText = sprintf(['Mean = %.2f\nMedian = %.2f\nSD = %.2f\n' ...
                     ...
                     '25th = %.2f | 75th = %.2f\nN = %d'], ...
                     mean_snr, median_snr, std_snr, ...
                     min_snr, max_snr, p25_snr, p75_snr, n_obs);

% Place near top-right corner of current axes (relative coords)
text(0.98, 0.95, statsText, ...
     'Units', 'normalized', ...
